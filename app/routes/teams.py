...
from app.models import User, Team, TeamMember, TeamRole, TeamStatus, TeamCompetition, Competition, CompetitionStatus, UserCompetition
from app.forms import TeamCreateForm, TeamEditForm, TeamInviteMemberForm, TeamJoinForm, TeamLeaveForm, TeamKickMemberForm, TeamCompetitionRegisterForm
import secrets
import string
...
@rate_limit_route(
    ...
    message="Too many team creation attempts.",
    methods={"POST"},
)
def create_team():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            if form.avatar.data:
                from app.services.file_upload import upload_team_avatar
                
                success, message, avatar_url = upload_team_avatar(
                    form.avatar.data,
                    team.id,
                    team.name
                )
                ...
                if success:
                    ...
                    db.session.rollback()
                    flash(f'Team avatar upload failed: {message}', 'danger')
...
def edit_team(team_id):
    ...
    if form.validate_on_submit():
        ...
        # Handle avatar upload if provided
        if form.avatar.data:
            from app.services.file_upload import upload_team_avatar, delete_file_from_s3
            from flask import current_app
            
            # Delete old avatar if exists and is from S3
            if team.avatar and 's3.amazonaws.com' in team.avatar:
                try:
                    old_key = team.avatar.split('.com/')[-1]
                    delete_file_from_s3(old_key, 'profiles')
                except Exception as e:
                    current_app.logger.warning(f"Failed to delete old team avatar: {str(e)}")
            ...
            if success:
                ...
                flash(f'Team avatar upload failed: {message}', 'warning')
...
@rate_limit_route(
    ...
    3600,
    identifier_func=user_or_ip_identifier,
    message="Too many team join attempts.",
    methods={"POST"},
)
...
@login_required
@rate_limit_route(
    "team_competition_register",
    10,
    3600,
    identifier_func=user_or_ip_identifier,
    message="Too many team competition registrations.",
    methods={"POST"},
)
...
@login_required
@rate_limit_route(
    "team_competition_unregister",
    10,
    3600,
    identifier_func=user_or_ip_identifier,
    message="Too many team competition unregistrations.",
    methods={"POST"},
)
