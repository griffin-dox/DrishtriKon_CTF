...
from app.models import User, UserRole, ChallengeType, CompetitionStatus, TeamStatus, TeamRole
from flask_wtf.file import FileField, FileAllowed, FileRequired
from app.security.user_security import validate_password_strength, PasswordBreachDetector
...
class ProfileForm(FlaskForm):
    ...
    avatar = FileField('Profile Picture', validators=[
        Optional(),
        SecureFileAllowed(['.jpg', '.jpeg'], mime_types=['image/jpeg'], message='Only JPEG images allowed'),
        FileSize(5 * 1024 * 1024, message='Profile picture must not exceed 5MB'),
        SafeFilename()
    ])
...
class ChallengeForm(FlaskForm):
    ...
    file = FileField('Attach File', validators=[
        ...
        SecureFileAllowed(
            ['.jpg', '.jpeg', '.pdf', '.zip'],
            mime_types=['image/jpeg', 'application/pdf', 'application/zip', 'application/x-zip-compressed'],
            message='Only JPEG, PDF, or ZIP files allowed'
        ),
        FileSize(50 * 1024 * 1024, message='Challenge file must not exceed 50MB'),
        SafeFilename()
    ])
...
class BadgeForm(FlaskForm):
    ...
    image = FileField('Badge Image', validators=[
        Optional(),
        SecureFileAllowed(['.jpg', '.jpeg'], mime_types=['image/jpeg'], message='Only JPEG images allowed'),
        FileSize(5 * 1024 * 1024, message='Badge image must not exceed 5MB'),
        SafeFilename()
    ])
...
class AdImageForm(FlaskForm):
    ...
    image = FileField('Ad Image', validators=[
        FileRequired(),
        SecureFileAllowed(['.jpg', '.jpeg'], mime_types=['image/jpeg'], message='Only JPEG images allowed'),
        FileSize(5 * 1024 * 1024, message='Ad image must not exceed 5MB'),
        SafeFilename()
    ])
...
class AdPlacementForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        from app.models import AdLocation
...
class TeamCreateForm(FlaskForm):
    ...
    avatar = FileField('Team Avatar', validators=[
        Optional(),
        SecureFileAllowed(['.jpg', '.jpeg'], mime_types=['image/jpeg'], message='Only JPEG images allowed'),
        FileSize(5 * 1024 * 1024, message='Team avatar must not exceed 5MB'),
        SafeFilename()
    ])
